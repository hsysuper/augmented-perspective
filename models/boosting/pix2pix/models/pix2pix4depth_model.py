...
class Pix2Pix4DepthModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, 64,
                                      'unet_1024', 'none', False, 'normal',
                                      0.02, self.gpu_ids)
    ...
    def set_input_train(self, input):
        ...
        self.outer = torch.nn.functional.interpolate(self.outer, (1024, 1024),
                                                     mode='bilinear',
                                                     align_corners=False)

        self.inner = input['data_inner'].to(self.device)
        self.inner = torch.nn.functional.interpolate(self.inner, (1024, 1024),
                                                     mode='bilinear',
                                                     align_corners=False)
        ...
        if self.isTrain:
            ...
            self.gtfake = torch.nn.functional.interpolate(self.gtfake,
                                                          (1024, 1024),
                                                          mode='bilinear',
                                                          align_corners=False)
    ...
    def set_input(self, outer, inner):
        ...
        inner = (inner - torch.min(inner)) / (torch.max(inner) -
                                              torch.min(inner))
        outer = (outer - torch.min(outer)) / (torch.max(outer) -
                                              torch.min(outer))
