...
def get_intrinsic_matrix(image_path: pathlib.Path) -> pathlib.Path:
    ...
    logging.debug(f"list of datasets: {dataset_dirs}, current image: {image_path}")
    for dataset_dir in dataset_dirs:
        if not image_path.is_relative_to(dataset_dir):
            logging.debug(f"{image_path} is not relative to {dataset_dir}, next one")
            continue
        logging.debug(f"{image_path} is relative to {dataset_dir}, trying to get intrinsic matrix")
        try:
            dataset = importlib.import_module(f"{script_dir.stem}.{dataset_dir.stem}")
